...
class DiT(nn.Module):
    ...
    def unpatchify(self, patches, patch_size, img_height, img_width):
    ...
    def forward(self, x, y, t):
        ...
        pos_embed = get_2d_sincos_pos_embed(embed_dim=x.shape[2], grid_size=H//self.p)
        x += torch.tensor(pos_embed, dtype=dtype).to(device)
...
def q3_b(train_data, train_labels, test_data, test_labels, vae):
    ...
    print(num_params)
    # load model
    #state_dict = torch.load('DiT_1.pth', map_location='mps')
